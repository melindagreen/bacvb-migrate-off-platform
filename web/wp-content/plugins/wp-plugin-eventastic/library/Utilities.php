...
class Utilities {
  ...
  */
  ...
  public static function getRecurrenceVersion () {
    ...
    $ro = get_option(SettingsAdminLayout::SETTING_PLUGIN_RECURRENCE_V2["key"], Constants::RECURRENCE_V2);
  ...
  */
  public static function getLatLngFromAddress ($address) {
    ...
    $geocoder = new \Eventastic\OpenCage\Geocoder\Geocoder($apiKey);
  ...
  public static function linkify ($text, $protocols=array('http', 'mail'), $attributes=array()) {
    ...
    */
    public static function include_template( $template, $attributes = []) {
        //$error = "Utilities::include_template >> Error Unknown";
        if( "filters" == $template ){
        ...
        if( $subTemplateFile ){
            $filePath = ( Utilities::is_file_in_theme( $subTemplateFile ) ) ? get_stylesheet_directory() . $subTemplateFile : realpath( __DIR__ . DIRECTORY_SEPARATOR . '../') . $subTemplateFile;
            include $filePath;
        }
        else{
            //echo $error;
...
//      error_log(  print_r( $postData , true) );
    ...
    */
    public static function recurrenceToString( $args ) {
      if( Utilities::getRecurrenceVersion() ){
        ...
        $startDate = ( array_key_exists( 'start_date', $meta)  && $meta['start_date'] ) ? date( strtotime( $meta['start_date'] ) ) : null;
        $endDate = ( array_key_exists( 'end_date', $meta)  && $meta['end_date'] ) ? date( strtotime( $meta['end_date'] ) ) : null;      
        $dates = $meta['pattern_dates'];
        $date_text = "";
        if( "custom" == $meta['repeat_pattern'] ){
          if( 'day' == $meta['repeat_type'] ){
            if( isset( $meta['repeat_number'] ) && $meta['repeat_number'] ){
              $date_text = "Every " . Utilities::numToOrdinalWord( $meta['repeat_number'] ) . " day";
            }
          }
          else{ 
            ...
            if( $pattern ){
                ...
                if( "monthly_occurence" == $pattern->freq ){
                    ...
                    foreach( $pattern->occurences as $m => $occurence){
                      $occursArray[] = Utilities::numToOrdinalWord($occurence);
                    };
                ...
                if( "monthly" == $pattern->freq ){
                    ...
                    $daysArray = [];
                    foreach( $pattern->days as $k => $day){
                      $daysArray[] = Utilities::numToOrdinalWord( $day );
                    };
                    $last = array_pop($daysArray);
                    $date_text .= count($daysArray) ? implode(", ", $daysArray) . " & " . $last : $last;
    ...
    */
    public static function datesToString( $args ) {
      ...
      if( Utilities::getRecurrenceVersion() ){
        $date_text = "";
        if( "pattern" == $meta['recurrence_options'] ){
          $recurrenceToString_args = [
            'post' => $post
          ];
          $recurrenceString = Utilities::recurrenceToString( $recurrenceToString_args );
          if( $recurrenceString ){
        ...
        elseif( "daily" == $meta['recurrence_options'] ){
          $day_seconds = 24*3600;
        ...
        elseif( "specific_days" == $meta['recurrence_options'] ){
    ...
    */
    public function getUpcomingOcccurences( $post ) {
      $meta = eventastic_get_event_meta($post->ID);
      $days = [];
      $html = "";
      $today = strtotime('today');
      $test_dates = [];
      if( isset($meta['recurrence_options'] ) ){
        if( "specific_days" == $meta['recurrence_options']  ){
          if( isset( $meta['repeat_dates']  ) ){
            $test_dates = $meta['repeat_dates'];
          }
        }
        if( "pattern" == $meta['recurrence_options']  ){
          if( isset( $meta['pattern_dates']  ) ){
            $test_dates = $meta['pattern_dates'];
          }
        }        
        if( "one_day" == $meta['recurrence_options']  ){
          $days[] = $meta['start_date'];
        }
        if( "daily" == $meta['recurrence_options']  ){
          $start_test_incr = strtotime($meta['start_date']);
          $end_test = strtotime($meta['end_date']);
      ...
      foreach( $test_dates as $repeatDate ){
        $test = date( strtotime($repeatDate) );
        if( $test >= $today ){
          $days[] = $repeatDate;
        }
      }
                          ...
                          foreach( $postData['eventastic_recurring_weeks'] as $weekNumber ){
                            if( array_key_exists('eventastic_recurring_days_v2', $postData) && is_array($postData['eventastic_recurring_days_v2']) ){
                              foreach( $postData['eventastic_recurring_days_v2'] as $dayName ){
                                $ordinal = Utilities::numToOrdinalWord($weekNumber);
