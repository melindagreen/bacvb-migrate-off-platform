...
const TaxonomyControl = ({
    ...
    return <div className='tax-control'>{(() => {
        ...
        switch (controlType) {
            case 'token':
                return <FormTokenField
                    ...
                    label={label}
                    suggestions={terms.map(term => term.name.replace('&amp;', '&'))}
                    ...
                    onChange={selected => onChange( 
                        selected.map(selectedTerm => terms
                            .filter(term => term.name.replace('&amp;', '&') === selectedTerm)
            ...
            default:
                return <SelectControl
                    ...
                    value={value[0]}
                    onChange={(selected) => onChange(selected === 'none' ? [] : [selected])}
                />
        }
    })()}</div>
}
