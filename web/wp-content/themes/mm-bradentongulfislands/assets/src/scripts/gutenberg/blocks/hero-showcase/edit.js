					...
					<>
						...
						<PanelBody title={__("Customize grid item")} initialOpen={false}>
							<p className="instructions">
								{__(
									"By default, the grid will pull in the selected post's title, thumbnail, and excerpt. To override these features, manually enter them here."
								)}
							</p>
										...
										render={({ open }) => (
											...
											>
												...
												{post?.factThumb?.sizes?.thumbnail?.url && (
													<img
														className="preview-thumb"
														src={post.factThumb.sizes.thumbnail.url}
													/>
												)}
												<Button
													onClick={open}
