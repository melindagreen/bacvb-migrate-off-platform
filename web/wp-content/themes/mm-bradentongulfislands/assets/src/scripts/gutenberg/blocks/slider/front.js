...
(function ($) {
	$(document).ready(function () {
		...
		const sliderBlockSelector =
			".wp-block-mm-bradentongulfislands-slider .swiper";
		...
		const createSwiper = (slider, index) => {
			...
			return new Swiper(slider, {
				...
				slidesPerGroup: slider.dataset.enableslidespergroup
					...
					: 1,
				slidesPerGroupAuto: slider.dataset.enableslidespergroupauto
					? true
					: false,
		...
		function changeInfoBlock() {
			...
			infoItems.map((item) => {
				let activeItem = $(
					".wp-block-mm-bradentongulfislands-slider .swiper-slide-active"
				);
				let itemText =
					$(".wp-block-mm-bradentongulfislands-slider .swiper-wrapper")
						.find(".swiper-slide-active article")
						.data(item) || "";

				if (itemText.length > 0) {
					$(`.slider-info-box #infoblock-${item}`)
			...
			if (buttonurl == "#" || buttonurl == " " || buttonurl.length < 1) {
				$(`.slider-info-box #infoblock-buttonurl`).addClass(
					"infoblock__item--hide"
				);
				$(`.slider-info-box #infoblock-buttonurl`).attr("href", "#");
				$(`.slider-info-box #infoblock-buttonurl`).attr(
					"aria-label",
					"click here to read more"
				);
			} else if (buttonurl.length > 0) {
				$(`.slider-info-box #infoblock-buttonurl`).removeClass(
					"infoblock__item--hide"
				);
