...
const Inspector = (props) => {
	...
	return (
		<InspectorControls>
			<PanelBody title="Default Thumbnail">
				<MediaControls {...props} />
			</PanelBody>
			<PanelBody title="Grid Settings">
				...
				{(postType === "listing" || postType === "event") && (
					<TaxonomyControl
						controlType="select"
						taxonomySlug={
							postType === "listing"
						...
						value={[preFilterCat]}
						onChange={(categories) =>
							setAttributes({ preFilterCat: categories[0] })
						}
					/>
				)}
				...
				{!preFilterCat && (
					<TaxonomyControl
						...
						value={[excludeCat]}
						onChange={(categories) =>
							setAttributes({ excludeCat: categories[0] })
						}
					/>
				)}
			...
			<PanelBody title={__("Filter Settings")}>
				...
				{(postType === "listing" || postType === "page") && (
					<TextControl
						...
						onChange={(catFilterSelections) =>
							setAttributes({ catFilterSelections })
						}
					/>
				)}
			</PanelBody>
