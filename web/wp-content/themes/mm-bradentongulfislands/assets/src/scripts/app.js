...
(function ($) {
	...
	function themeOnLoad() {
		...
		scrollAnimation(
			...
			400,
			"is-style-heading-shadow--shadow",
			true
		);
		...
		scrollAnimation(
			".share-the-love-bg::after",
			400,
			"share-the-love-bg::after--zoom-in",
			true
		);
		scrollAnimation(
			".share-the-love-bg::before",
			400,
			"share-the-love-bg::before--zoom-in",
			true
		);
		...
		$(".fade-in-children").each(function (index, el) {
			...
			$(el)
				...
				.each(function (childIndex, childEl) {
					$(childEl).css("transition-delay", `${(childIndex + 1) * delay}s`);
				});
		});
		...
		if (typeof gform !== "undefined" && $("#gform_5").length) {
				...
				(repeatIndex, repeatItem) => {
					$(repeatItem)
						...
						.each((fieldIndex, fieldItem) => {
							...
							if (name.includes("[]"))
								name = name.replace("[]", `[${repeatIndex}][]`);
							else name += `[${repeatIndex}]`;
							$(fieldItem).attr("name", name);
							$(fieldItem).attr(
								"id",
								$(fieldItem).attr("id") + `-${repeatIndex + 1}`
							);
						});
				}
			);
			...
			gform.addAction(
				...
				function (elem, formId, fieldId) {
					if (parseInt(formId) === 5 && parseInt(fieldId) === 32) {
						toggleDateRepeater();
					} else if (
						parseInt(formId) === 5 &&
	...
	jQuery
		.get(
			"https://maddencdn.com/global/code/weather/getWeatherData.php?cityName=Bradenton",
			function () {}
		)
		.done(function (data) {
			var forecastHTML = "";
			try {
				...
				if (mainWeather === "Clear") {
					...
					forecastHTML +=
						'<img src="/wp-content/themes/mm-bradentongulfislands/assets/images/weather-icons/cloud-sun-rain-solid.png" alt="rain" />';
				} else if (mainWeather === "Clouds" || mainWeather === "Clouds") {
					forecastHTML +=
						'<img src="/wp-content/themes/mm-bradentongulfislands/assets/images/weather-icons/cloud-sun.png" alt="cloudy" />';
				} else if (mainWeather === "Thunderstorm") {
					forecastHTML +=
						'<img src="/wp-content/themes/mm-bradentongulfislands/assets/images/weather-icons/hurricane-solid.png" alt="chancerain" />';
				} else {
					forecastHTML += night
						? '<img src="/wp-content/themes/mm-bradentongulfislands/assets/images/weather-icons/moon-cloud.png" alt="clear" />'
						: '<img src="/wp-content/themes/mm-bradentongulfislands/assets/images/weather-icons/sun-regular.png" alt="sunny" />';
				}
				forecastHTML += `<p class="header-weather-forecast__temp">${
					weather["temp"]
				}${weather["temp"] > 99 ? "<br>" : ""}&deg;F</p>`;
			} catch (err) {}
	...
	function queryCarouselOnMobile() {
		...
		if (getIsSmall() && blockQueryLoop.length > 0) {
			...
			if (slides.length > 0) {
				const swiperWrapper = $(
					'<div class="wp-block-post-template swiper-wrapper"></div>'
				);

				slides.addClass("swiper-slide").appendTo(swiperWrapper);

				const swiperContainer = $(
					'<div class="wp-block-query swiper-container"></div>'
			...
			if (swiperContainer.length > 0) {
				...
				slides
					...
					.appendTo(blockQueryLoop.find(".wp-block-query__content"));
			}
		}
	}
	...
	function themeOnScroll() {
		...
		scrollAnimation(
			...
			0.75,
			"is-style-arrow-button--rotate"
		);
		scrollAnimation(
			".grid-item-body--2 .grid-item-body__arrow",
			0.75,
			"grid-item-body__arrow--forward"
		);
		scrollAnimation(
			".grid-item-body--3 .grid-item-body__arrow",
			0.75,
			"grid-item-body__arrow--rotate"
		);
		scrollAnimation(
			".grid-item-body--1 .grid-item-body__arrow",
			0.75,
			"grid-item-body__arrow--rotate"
		);
	}
