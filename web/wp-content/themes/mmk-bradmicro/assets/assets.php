...
namespace MaddenTheme\Assets;

use MaddenTheme\Library\Constants as C;
use MaddenTheme\Library\Utilities as U;
use Mustache_Engine;
    ...
    if (is_admin()) {
      ...
      wp_enqueue_script(
        ...
        false // in footer?
			...
			wp_add_inline_script( C::THEME_PREFIX . "-admin-js", 'const maddenThemeData = ' . json_encode( array(
  ...
  public static function enqueue_front_scripts() {
    ...
    foreach ($scripts as $k => $v) {
      wp_enqueue_script(
        ...
        filemtime( get_stylesheet_directory() . $v ), // version
        array(
            'strategy'  => 'defer',
            'footer'    => true
        )
