...
const registerAllBlockStyles = () => {
	//setTimeout is currently required to remove core block styles
	//https://github.com/WordPress/gutenberg/issues/40202
	//https://github.com/WordPress/gutenberg/issues/27607
	setTimeout(() => {
		for (const block in coreStyles) {
			// Unregister unwanted core styles
			if (coreStyles[block].remove && coreStyles[block].remove.length > 0) {
