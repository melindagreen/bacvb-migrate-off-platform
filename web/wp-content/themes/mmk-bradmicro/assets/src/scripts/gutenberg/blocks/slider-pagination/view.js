...
/**
 * Add any sliders found on the page to the global registry if they're not a glready there
 */
	...
	sliderElements.forEach((sliderSection) => {
		...
		if (sliderId && !window.maddenTheme.sliders[sliderId]) {
			...
			if (swiperElement && swiperElement.swiper) {
				...
				if (debug)
					console.log(`Added existing Swiper to registry: ${sliderId}`);
...
function initSliderControls() {
	try {
		...
		const controlBlocks = document.querySelectorAll(
			`.wp-block-${THEME_PREFIX}-slider-pagination`
		);
		...
		const pageSliders = document.querySelectorAll(
			`.wp-block-${THEME_PREFIX}-slider`
		);
		if (debug)
			console.log(`Found ${pageSliders.length} slider elements for pagination`);
		...
		controlBlocks.forEach((controlBlock) => {
			try {
				...
				const sliderNumber = parseInt(
					controlBlock.getAttribute("data-slider-number"),
					10
				);
				...
				if (
					isNaN(sliderNumber) ||
					sliderNumber <= 0 ||
					sliderNumber > pageSliders.length
				) {
				...
				if (!targetSliderId || !window.maddenTheme.sliders[targetSliderId]) {
					if (debug)
						console.log("Target slider ID not found in maddenTheme.sliders");
					return;
				}
				...
				const paginationEl = controlBlock.querySelector(
					".external-swiper-pagination"
				);
				if (paginationEl) {
				...
				const prevArrowEl = controlBlock.querySelector(
					".external-swiper-button-prev"
				);
				const nextArrowEl = controlBlock.querySelector(
					".external-swiper-button-next"
				);
				...
				if (prevArrowEl && nextArrowEl) {
					...
					// Apply border color
					if (arrowBorderColor) {
						prevArrowEl.style.border = `2px solid ${arrowBorderColor}`;
						nextArrowEl.style.border = `2px solid ${arrowBorderColor}`;
					}
...
function updateExternalPagination(paginationEl, swiper) {
	...
	if (debug)
		console.log(`Total slides: ${totalSlides}, Active index: ${activeIndex}`);
	...
	for (let i = 0; i < totalSlides; i++) {
		...
		if (dotBorderColor) {
			bullet.style.border = `2px solid ${dotBorderColor}`;
		}
