...
export const initInteractiveMap = () => {
	...
	const handleMove = (e) => {
		...
		if (Math.abs(deltaX) > panThreshold && !touchMoved) {
			...
			if (!hasSwipedOnce && !hasViewedOnce) {
				...
				const $interactivesG = $(
					".wp-block-madden-theme-bradenton-meetings-map svg #interactive > g"
				);
				$interactivesG.addClass("bounce-scale");

				// Remove the animation class after it finishes
				setTimeout(() => {
					$interactivesG.removeClass("bounce-scale");
	...
	const $mapContainer = $(".wp-block-madden-theme-bradenton-meetings-map"); // Assuming this is the main container you want to observe for viewport
	...
	$interactivesG.on("click", function (e) {
		if (touchMoved) {
			e.preventDefault();
			e.stopImmediatePropagation();
			return;
	...
	$("#insetTrigger")
		.on("mouseenter", function () {
			$("#insetHighlight").addClass("active");
		})
		.on("mouseout", function () {
			$("#insetHighlight").removeClass("active");
		});
};
