...
const SliderOptions = (props) => {
	...
	const wrapperProps = useBlockProps({
		className: "wp-block-" + THEME_PREFIX + "-slider",
	});
					...
					<>
						...
						{attributes.enableSlidesPerViewAuto && (
							<UnitControl
								...
								help={__("Set a slide width for automatic slides per view.")}
								value={attributes.slidesPerViewWidth}
								min={1}
								onChange={(val) => {
									setAttributes({ slidesPerViewWidth: val });
								}}
							/>
						)}
						{!attributes.enableSlidesPerViewAuto && (
						...
						<ToggleControl
							...
							help={__("If using content card, this will add styles to force equal height to the tallest slide")}
