# Madden Media Madre Parent Theme

This parent theme provides global functionality and some sensible defaults upon which a child theme may expand. It is not intended to serve as a full WordPress theme on its own. Updates to this theme should be made only if they're desireable for all Madden sites.
## Useful links
...
* `assets` contains all buildable files and configuration for building theme.
    ...
    * `images` holds static theme images such as UI icons, theme logos, etc.
...
* `library` contains PHP functions that load and customize the theme and provide expanded functionality to the child theme.
## Building this theme
This theme's `composer.json` file contains several useful commands for building out the theme's assets. These commands can also be run directly from within the assets directory.
