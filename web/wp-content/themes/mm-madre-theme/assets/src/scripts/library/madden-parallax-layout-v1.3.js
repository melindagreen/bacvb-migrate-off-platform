...
(function ($) {
			...
			$(window).scroll(function () {
				...
				var x =
					settings.adjustX == 0
						...
						: Math.round(
								(offsetTop - scrollTop) * settings.adjustX
						  ) +
						  0 +
						  settings.bgXOffset +
						  'px';
				var y =
					settings.adjustY == 0
						...
						: Math.round(
								(offsetTop - scrollTop) * settings.adjustY
						  ) +
						  0 +
						  settings.bgYOffset +
						  'px';
	...
	//
	// Simple zoom effect for background content in a div
	//
	$.fn.zoomBG = function (options) {
		var windowHeight = $(window).height();
		var lastScrollTop = 0;
		var scrollTop = $(window).scrollTop();
		var offsetTop = 0;
		...
		//
		// zoomOut: the amount to zoom out to from the starting point (assumes start is 100%)
		// zoomIn: the amount to zoom in to from the starting point (assumes start is 100%)
		// zoomRate: how much to adjust zoom each iteration (in percent)?
		// zoomOn: which axis to zoom?
		// bgSizeX: what to make the bg size on the X axis (usually "auto" if not zooming this direction)
		// bgSizeY: what to make the bg size on the Y axis (usually "auto" if not zooming this direction)
		//
		var settings = $.extend(
			{
				zoomOut: 0,
		...
		return this.each(function () {
			var $this = $(this);
			var $bgAxisPos = settings.zoomOn == 'X' ? 0 : 1;

			// handle the scroll
			$(window).scroll(function () {
				// cache these first to use locally
				var zoomRate = settings.zoomRate;
				var bgSizes = $this.css('backgroundSize').split(' ');
				var bgSize = parseInt(
					Number(bgSizes[$bgAxisPos].replace('%', ''))
				);
				...
				// reverse the direction of the zoom based on scroll direction
				if (scrollTop < lastScrollTop && zoomRate != 0) {
					zoomRate = zoomRate * -1;
				}
				...
				if (
					...
					// ...we're outside the viewport of this item
					return;
				} else if (z >= settings.zoomIn || z <= settings.zoomOut) {
					// ...or if we are at the min or max
					return;
				}
	...
	$.fn.parallaxLockElement = function (options) {
		...
		return this.each(function () {
			...
			// handle the scroll
			$(window).scroll(function () {
				var parentElTopViewOffset = parseInt(
					$parentEl.offset().top -
						$(window).scrollTop() -
						settings.offsetTop
				);
				var parentElBottomViewOffset =
					$parentEl.offset().top + $parentEl.outerHeight();
				var lockElHeight = parseInt($this.outerHeight());
				var parentThisBottomDiff =
					$parentEl.offset().top +
					$parentEl.outerHeight() -
					($this.offset().top + $this.outerHeight());

				if (parentElTopViewOffset <= 0) {
					// it's in range - are we beneath where it should be locked, though?
					if (
						parentElBottomViewOffset + parentThisBottomDiff <=
						...
						if (elLocked) {
							...
							$this.css({
								position: 'absolute',
								top: 'inherit',
								bottom: parentThisBottomDiff + 'px',
							});
	...
	$.fn.parallaxAnimateElement = function (options) {
		...
		if (settings.showWhenCentered) {
			...
			if (settings.centerOffsetAnimationRange == null) {
				...
				settings.centerOffsetAnimationRange =
					parseInt(settings.centerOffsetAnimationRange) / 100;
			} else {
				// ignore it
				settings.centerOffsetAnimationRange = 1;
			}
			...
			if (settings.endOffset != null) {
				...
				if (settings.endOffset.indexOf('%') != -1) {
					...
					endOffsetLoc = Math.floor(
						parentHeight - parentHeight * parsedSO
					);
				} else {
					// px
		...
		return this.each(function () {
			...
			$(window).scroll(function () {
				...
				var inViewport =
					elOT + $this.outerHeight() <= $(window).scrollTop() ||
				...
				if (inViewport) {
					...
					if (settings.showWhenCentered) {
						...
						var selfViewportOffset = parseInt(
							$this.offset().top - $(window).scrollTop()
						);
						var selfCenter = parseInt(
							selfViewportOffset + selfHeight / 2
						);
						var windowCenter = parseInt(
						...
						if (windowCenter < selfEnter) {
							...
							windowCenter >= selfEnter &&
							windowCenter <= selfExit
						) {
							// yes - figure out the percent inside the range (capping at 0/1)
							var percentIn = Math.max(
								0,
								Math.min(
									1,
									1 -
										(selfEnter +
											(selfCenter - windowCenter)) /
											selfCenter
								)
							);
						...
						var parentViewportOffset = parseInt(
							$this.parent().offset().top - $(window).scrollTop()
						);
					...
					$this.attr(
						...
						settings.elementClass +
							' ' +
							settings.elementFrameClassRoot +
							showFrame
					);
				}
			});
