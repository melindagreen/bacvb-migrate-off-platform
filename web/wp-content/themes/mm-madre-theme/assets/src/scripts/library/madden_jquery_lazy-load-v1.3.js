 ...
 *    - Single element by ID, from framework: getLazyLoadPluginObject().manual('[ID]');
 */
...
(function ($) {
	...
	$.fn.lazyLoad = function (options) {
		var IS_BOT = /bot|google|baidu|bing|msn|duckduckgo|teoma|slurp|yandex/i.test(
			navigator.userAgent
		);
		...
		this.each(function () {
			if (IS_BOT || $(this).data('load-onload')) {
				...
				// we need to also store these so they're called in the resize event
				onloadEls.push($(this));
			} else {
		...
		$(window).on('orientationchange resize', function () {
			...
			// reload the onload element too
			for (var i = 0; i < onloadEls.length; i++) {
				loadContent(onloadEls[i], settings.debug);
			}
		});
		...
		$(window).scroll(function () {
			...
			for (var i = 0; i < lazyLoadEls.length; i++) {
				...
				var size = lazyLoadEls[i].data('load-offset')
					? lazyLoadEls[i].data('load-offset')
					: 'sm';
				var offset = 0;
				var manual = lazyLoadEls[i].data('load-manual');
				switch (size) {
					case 'lg':
				...
				if (!manual && scrollTop >= offsetTop - offset) {
					// yes!
		...
		this.manual = function (id) {
			for (let i = 0; i < lazyLoadEls.length; i++) {
				if (lazyLoadEls[i].attr('id') === id) {
					loadContent(lazyLoadEls[i], settings.debug);
					processedLazyLoadEls.push(lazyLoadEls[i]);
					lazyLoadEls.splice(i, 1);
				}
	...
	function getDocumentSize() {
		_isSmall =
			jQuery(DEFAULT_SM_TEST_EL).length != 0 &&
			jQuery(DEFAULT_SM_TEST_EL).css('float') != 'none'
				? true
				: false;
		_isMedium =
			jQuery(DEFAULT_MD_TEST_EL).length != 0 &&
			jQuery(DEFAULT_MD_TEST_EL).css('float') != 'none'
				? true
				: false;
		_isLarge =
			jQuery(DEFAULT_LG_TEST_EL).length != 0 &&
			jQuery(DEFAULT_LG_TEST_EL).css('float') != 'none'
				? true
				: false;
	}
	...
	function loadContent(el, debug) {
		...
		if (sizeData) {
			...
			if (el.data('load-type') == 'img') {
				...
				el.data('load-type') == 'class' &&
				!el.hasClass(sizeData)
			) {
				// css class
			...
			} else if (el.data('load-type') == 'bg') {
				// background-image property transfer
				el.css('background-image', "url('" + sizeData + "')");
				if (debug) {
			...
			if (debug) {
				console.log(
					'LAZYLOAD not loading: no size found for this view'
				);
				console.log(el.data());
			}
		}
