...
sub vcl_recv {
    ...
    set req.http.Host = regsub(req.http.Host, ":[0-9]+", ""); # Remove port number from host
    unset req.http.proxy; # Remove any proxy headers
    set req.url = std.querysort(req.url); # Sort query parameters alphabetically for consistent caching
    set req.url = regsub(req.url, "\?$", ""); # Remove trailing question mark
    set req.http.Surrogate-Capability = "key=ESI/1.0"; # Declare ESI capabilities

    # Grace mode setup for healthy backends
    if (std.healthy(req.backend_hint)) {
        set req.grace = 10s; # Set grace period for stale content
    }
...
sub vcl_hash {
    # Custom hashing for cache variations based on specific request headers
...
sub vcl_deliver {
    ...
    if (resp.http.X-Streaming == "Y-Stream") {
        ...
        # You can also control cache-related headers for streaming content
        # For example, you might want to set Cache-Control to "no-cache"
